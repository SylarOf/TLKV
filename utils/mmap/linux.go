...
// mmap uses the mmap system call to memory-map a file.If writeable is true, 
// memory protection of the pages is set so that they may be writeen to as well
func mmap(fd *os.File, writeable bool, size int64) ([]byte, error) {
	mtype := unix.PROT_READ
	if writeable {
		mtype |= unix.PROT_WRITE
	}
	// second par o is starting offset
...
func mremap(data []byte, size int) ([]byte, error) {
    ...
    // Get the pointer and length of the byte slice using unsafe.
    ...
    // Convert the new mmap address back to a byte slice.
    newData := unsafe.Slice((*byte)(unsafe.Pointer(mmapAddr)), size)
    return newData, nil
}
